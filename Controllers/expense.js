  ...
  const postData = async () => {
    try {
      ...
      await req.user.createExpense(
        { amount: quantity, description, category },
        { transaction: t }
      );
...
exports.deleteExpense = (req, res) => {
  ...
  const deleteExpense = async () => {
    ...
    try {
      const toBeDeletedExpense = await Expense.findOne(
        { where: { userId: req.user.id, id: expenseId } },
        { transaction: t }
      );
...
exports.allDownloadedExpenses = async (req, res) => {
  try {
    const allDownloadedExpenses = await Expensedownload.findAll({
